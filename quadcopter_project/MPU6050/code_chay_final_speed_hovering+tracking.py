...
import Tracking_Thread_lib_3 as Tracking_Thread_lib
import OpticalFlow_Thread_lib_2 as OpticalFlow_Thread_lib
...
#         file.write("roll,pitch,heading,pressure,throttle\n")
...
#Create Thread
num = Value("d",0.0)
TrackingThread = Process(target=Tracking.run,args=[num])
...
# while Tracking.tracking_init() == 0:
#     time.sleep(1)
mpu6050.flushFIFO()
time.sleep(FULL_FIFO_BATCHES/sampling_rate)
ya = 0.0
...
#                 mpu6050.flushFIFO()
        mpu6050.flushFIFO()
        counter += 1
        (PID_pressure, altitude) = MS5611.readValue(counter,MIX)
        if PID_pressure>10:
            PID_pressure=10
    ...
    #             pressure1 = round(pressure/100,3)
        ...
        if gear == 1 and run == 1:
            if MIX == 2:
                ...
                x_pos, y_pos = OpticalFlow.position()
            else:
                OpticalFlow.disable_position_mode()
                x_pos = 0
                y_pos = 0
            if MIX == 1:
                TrackingThread.terminate()
    ...
    #             off_rps=0
        ...
        ser.write(b'%f %f %f %f %f %f %f\r\n'%(roll,pitch,x_pos,y_pos,nfb,elapse_time,motion_dt))
    ...
    #         print(roll,pitch,throttle,roll_setpoint,pitch_setpoint,nfb,elapse_time,lipo_volt,yr_target)
        print(roll,pitch,roll_setpoint,pitch_setpoint,nfb,throttle,lipo_volt,PID_pressure,x_pos,y_pos,pa_target,ra_target,num.value)
