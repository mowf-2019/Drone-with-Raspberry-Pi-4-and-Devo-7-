...
mpu6050 = GY_86_lib.MPU6050(0x68, 1, 1)
mpu6050.flushFIFO()
...
while True:
    ...
    if elapse_time > (1/motion_rate):
        ...
        qax, qay, qaz, qrx, qry, qrz = mpu6050.scaleSensors(qax, qay, qaz, qrx, qry, qrz)
        ra, pa = Rotation_lib.GetRotationAngles(qax, qay, qaz)
        roll  = 0.98*(roll + qrx*motion_dt) + 0.02*(ra)
        pitch = 0.98*(pitch + qry*motion_dt) + 0.02*(pa)
        
        if gear == 1:
            #======================================================================================
            # Angle PIDs
            #======================================================================================
            [p_out, i_out, d_out] = pa_pid.Compute(pitch, 0, motion_dt)
            pr_target = p_out + i_out + d_out

            [p_out, i_out, d_out] = ra_pid.Compute(roll, 0, motion_dt)
            rr_target = p_out + i_out + d_out

            [p_out, i_out, d_out] = ya_pid.Compute(0, 0, motion_dt)
            ...
            [p_out, i_out, d_out] = yr_pid.Compute(qrz, 0, motion_dt)
...
#         z=mpu6050.readheading(roll*(math.pi)/180,pitch*(math.pi)/180)

    if elapse_time2>1:
        end2=time.time()
        print(qax, qay, qaz, roll, pitch)
