...
#             PID_pressure=-10
        ...
        if gear == 1 and run == 1:
            ...
            # Angle PIDs
            #======================================================================================
            [p_out, i_out, d_out] = pa_pid.Compute(pitch, pitch_setpoint, motion_dt, gear, throttle)
            pr_target = p_out + i_out + d_out

            [p_out, i_out, d_out] = ra_pid.Compute(roll, roll_setpoint, motion_dt, gear, throttle)
...
#         ser.write(b'%f %f\r\n'%(PID_pressure,error_press))
#         file.write(str(roll)+","+str(pitch)+","+str(heading)+","+str(pressure1)+","+str(throttle)+"\n")
...
#         print(roll,pitch,throttle,roll_setpoint,pitch_setpoint,nfb,elapse_time,lipo_volt,yr_target)
        print(roll,pitch,throttle,heading,yaw_error,lipo_volt,PID_pressure,altitude)
