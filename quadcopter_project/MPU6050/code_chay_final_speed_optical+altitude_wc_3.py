...
import OpticalFlow_Thread_lib_4 as OpticalFlow_Thread_lib
...
#         file.write("roll,pitch,yaw,throttle,altitude,xpos,ypos\n")
...
velo_x_pre = 0
velo_y_pre = 0
def opticalflow_calc(velo_x,velo_y,count):
    global velo_x_pre, velo_y_pre
    diff_x = round(velo_x,3) - velo_x_pre
    diff_y = round(velo_y,3) - velo_y_pre
    
    velo_x_pre = round(velo_x,3)
    velo_y_pre = round(velo_y,3)
    if diff_x != 0:
        velo_x_add = diff_x/10.0
    if diff_xy!= 0:
        velo_y_add = diff_y/10.0
...
# 
# print(max_mgx,min_mgx,max_mgy,min_mgy,max_mgz,min_mgz)
max_mgx = 752.56
min_mgx = -30.36
max_mgy = 538.2
min_mgy = -287.04
...
try:
        ...
        #             pressure1 = round(pressure/100,3)
            ...
            if gear == 1 and run == 1:
                if MIX == 2:
                    ...
                    pa_target, ra_target, x_pos, y_pos = OpticalFlow.position()
                else:
    ...
    #                 esc_4_rps = throttle - yr_out + off_rps
        ...
        #         ser.write(b'%f %f %f %f %f %f\r\n'%(qax,qay,qaz,qrx,qry,qrz))
            opticalflow_calc(x_pos,y_pos)
#             file.write(str(roll)+","+str(pitch)+","+str(yaw1_angle)+","+str(throttle)+","+str(h)+","+str(x_pos)+","+str(y_pos)+"\n")
        #         ser.write(b'%f %f\r\n'%(roll, pitch))
                

        if elapse_time2>1:
            end2=time.time()
#             print(roll,pitch,yaw1_angle,nfb,throttle,lipo_volt,actual_pressure,PID_pressure,h,pa_target,ra_target,x_pos,y_pos)
