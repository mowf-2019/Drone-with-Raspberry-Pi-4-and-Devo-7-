...
def PID_yaw(gyroZ,yaw_setpoint,dt):
    ...
    pid_yaw_output = Kp_yaw*yaw_error + Ki_yaw*yaw_i*dt + Kd_yaw*(yaw_error - last_yaw_error)/dt
...
#             pid_pitch_setpoint=0
        ...
        if gear==1:
            roll_ia += (roll-roll_setpoint)*dt
            pitch_ia += (pitch-pitch_setpoint)*dt
            pid_roll_setpoint = -((roll-roll_setpoint)*1 + 1*roll_ia)
            pid_pitch_setpoint = -((pitch-pitch_setpoint)*1 + 1*pitch_ia)
            pid_yaw_setpoint = -yaw*0.5
        else:
            pid_roll_setpoint=0
            pid_pitch_setpoint=0
            roll_ia=0
        ...
        PIDyaw=PID_yaw(gyroZ,pid_yaw_setpoint,dt)
...
#         print(lipo_volt,yaw,esc_1_rps,esc_2_rps,esc_3_rps,esc_4_rps)
        print(roll,pitch,yaw,PIDpitch,PIDroll,throttle)
        end2=time.time()
