...
def PID_roll(gyroX,roll,roll_setpoint,dt):
...
def PID_pitch(gyroY,pitch,pitch_setpoint,dt):
...
def PID_yaw(gyroZ,yaw,yaw_setpoint,dt):
...
while True:
    ...
    if (dt >= 0.01):   #Sample rate 100Hz
        ...
        gyroY = gyroY_pre*0.8 + 0.2*Yrate #LPF
        gyroZ = gyroZ_pre*0.8 + 0.2*Zrate #LPF
        gyroX_pre=gyroX
        gyroY_pre=gyroY
        gyroZ_pre=gyroZ
        ...
        PIDroll=PID_roll(gyroX,roll,roll_setpoint,dt)
        PIDpitch=PID_pitch(gyroY,pitch,pitch_setpoint,dt)
        PIDyaw=PID_yaw(gyroZ,yaw,0,dt)
